...
Since 2026-08-31 the file carries the **30.8 × 54 barrel** (the card shelf, Version 2's
...
- **The v9 update (2026-09-24; decisions D-1, D-2, D-6).** Three more constants of this
  ...
  `cylinder.bottom_chamfer` and `gears.axis_cuts` (vent, then the socket cone); the owner of
...
| 2026-08-31 | 1.6 | **The 54 mm barrel becomes Version 2's alone.** Brennen's deployment verdict (same day as v1.4): the project-wide default cylinder returns to the 52 mm Version 1 standard — the height every previously shipped V1 gear model pairs with, and the size the integrated-gears BETA is hard-gated to — while `V2_BARREL_HEIGHT_MM` stays 54 and `V2_PRESET_OVERRIDES` still forces 30.8 × 54 onto the dials whenever Version 2 is selected. Nothing in this document's own numbers changes; v1.4's "matches the new project-wide cylinder default" clause is superseded. Height still tells Version 2 (54) apart from the gears BETA and the V1 default (52). Version 2's own OpenSCAD companion followed to 54 later the same day (v1.7); the Version 1 `.scad` files keep the 52 mm barrel, and the updated (v7.2) gears ship outside MakerWorld. Still not print-tested at 54. |
