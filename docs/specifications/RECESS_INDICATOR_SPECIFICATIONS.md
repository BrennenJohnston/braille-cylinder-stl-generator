...
Embosser Version 2 (prototype) recommends **one cell fewer in visual mode**: 12 with indicator letters on, 13 with them off. Its barrel is 30.1 mm rather than 30.75/30.8, which is 2.2 mm less circumference — in visual mode exactly one cell's worth. At 13 text cells plus 2 marker columns the seam gap is 3.6 mm where a cell's dots need 4.0 mm, so the old recommendation named a layout `checkPhysicalFit()` warns against on the same screen; at 12 the gap is 10.1 mm. **Tactile mode is unchanged at 14** — it needs 9.0 mm and still gets 10.1 mm. Version 1 recommendations are untouched. Held in `updateGridColumnsForPlateType()` in `public/index.html`, gated on `isVersion2()`. See EMBOSSER_VERSION_2_KEYED_CUTOUTS_SPECIFICATIONS.md.
