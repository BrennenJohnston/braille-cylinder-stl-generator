...
| C5 | Task controls come before, or close after, utility controls | **FAIL, closed by decision** | **14 of 32** stops still precede `placement_mode_auto`, and now will not change. This is **F-F**, which Brennen CLOSED on 2026-08-23 after hearing the page: the skip links make the count irrelevant to him. The criterion still fails on its own terms and is recorded failing rather than reworded to pass |
...
| C7 | No description restates its label; no `sr-only` duplicates visible text | **~~FAIL~~ PASS 2026-08-23** | **F-P FIXED** (`255f725`): the redundant `title` is off all three font-size buttons, and the AX tree now reports no node whose name equals its description. Recorded honestly: the predicted *"Decrease font size, button, Decrease font size"* **never occurred** in Brennen's 1,799-utterance run — NVDA suppresses a description identical to the name, proven in the same run by the theme toggle, whose DIFFERENT description was spoken in full. Removed anyway, because JAWS and VoiceOver need not de-duplicate. Cost he accepted: no hover tooltip on those three icon buttons |
| C8 | Everything in the tab ring is keyboard-operable; value controls announce their range | **~~PASS, with one open item~~ PASS 2026-08-25** | every ring member is a native `a`/`button`/`input`/`select`/`textarea` — no custom key handling needed anywhere. Ranges: 33/33 dials bounded (item E/H). The open item — `cylinder_diameter_mm` invalid from its own default, `value="30.75"` against `min="10" step="0.1"` — is **CLOSED**: Brennen chose to move the step to `0.05` rather than the value, so 30.75 stays the default everywhere and the schema/backend layer never moved. The `constraintValidation.spec.ts` seed that existed only to work around it is gone too |
...
*(Updated 2026-08-25: C8's open item is closed, so the score now reads 7 pass, 2 fail,
1 that only a person can answer.)*
...
|---|---|---|
| 1.11 | 2026-08-23 | **The walkthrough was RUN, and it closed the finding no probe could close while finding three the probes had all passed.** **F-F CLOSED** — Brennen answered Part 4: with the skip links working, ring length no longer matters to him. It closes as *addressed by other means*, not by the number moving; it is still 14 of 32 and C5 is recorded still FAILING on its own terms rather than reworded to pass. The FD-27b column reorder is off the table. **F-P CLOSED** (`255f725`) and C7 flips to PASS — with the correction that **the predicted double-speech never happened**: NVDA 2026.1.1 suppresses a description identical to the accessible name (proven by the theme toggle's *different* description being spoken in full in the same capture), so F-P was real in the tree and inert in NVDA, and was fixed for JAWS/VoiceOver rather than for anything measured. **Three NEW findings from the run, all fixed the same day:** **F-Q** (`e62a2bd`) the Help & Guide dialog leaked focus into Chrome's own toolbar after three Tabs — the trap's selector never filtered for what can take focus, so with seven panels and one shown, `lastElement` sat in a `hidden` panel and the wrap could never fire; page-behind focus **41 of 44 presses → 0** on WebKit, and a second defect surfaced in the close path (Safari does not focus a button on click, so focus was stranded on the hidden dialog). **F-R** (`b642e7a`) generation and download succeeded silently with warnings live — no generate path consulted any warning box — and the plate embossed correctly **missing the tail of the text**, because Translate to Braille truncates to what fits (4 rows of 10/12/13/12 cells) and a filled braille field is then used exactly as written. **F-S** (`ddd7bd8`) the shipped default tripped the crowding warning permanently; the 0.50 mm reliable line turned out to have **no stated basis anywhere**, and is now a documented-provisional 0.45 pending a print test that walks the gap to visible failure. Bar after all of it: ruff clean, **140 pytest, 2 vitest, 217 e2e** (chromium+firefox+webkit), Nu 0/0 source and rendered, Lighthouse 100/100, contrast 0, reflow 0 of 6. **Still not recorded in Brennen's own words: whether the page is easier to USE.** |
