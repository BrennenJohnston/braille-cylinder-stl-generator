...
too big to fit (at the second 9) — not one per digit. Its exact wording is under
revision (it currently mixes cells and columns; see the cells-vs-columns finding
in the project memory), so listen for the count, not the sentence.
