...
### Changed
- **The vendored OpenSCAD copy is refreshed to upstream v2.9.0** (`OpenSCAD/`, 2026-09-23), so the downloadable OpenSCAD file speaks this app's language: one render builds both cylinders, as Generate does here; its Customizer sections carry this app's names (Card Sides, Gears, Cylinders to Generate, Row Indicator Style, Card Thickness); the BETA and sign-off labels are gone; and its gear hardware note is this app's current one. Each plate's geometry is unchanged and no OpenSCAD parameter was renamed, so saved Customizer settings keep working. The parameter mapping, the MakerWorld quick start and its PDF are refreshed with it, and `OpenSCAD/README.md` describes the pair default.
