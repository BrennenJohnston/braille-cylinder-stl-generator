...
### Changed
- **Each Version 2 gear peg has its own key clearance dial, and the default is 0.095 mm** (2026-09-25). Under Expert Mode → Surface Dimensions → Version 2 Keyed Cutouts the single Key clearance dial is replaced by four — gear A1 (top of Cylinder A), A2 (bottom of Cylinder A), B1 (top of Cylinder B) and B2 (bottom of Cylinder B) — so one gear's fit can be tightened or loosened without moving the other three. Every dial starts at 0.095 mm, the value Brennen's print round confirmed for A2 and B2 (cylinders on Bambu Studio's 0.12 mm Fine Detail preset, gears on 0.2 mm Strength); the old 0.110 default had printed a touch loose at those settings. Requests carry `v2_key_clearance_a1_mm` … `_b2_mm`; a request or saved preference from before this change, which carries only the shared `v2_key_clearance_mm`, still applies that one value to all four holes. The anti-rotation nub and sockets do not move with any dial. The Version 2 golden fixtures were regenerated at the new default; Version 1 and the fused Version 2 roller are unchanged.
...
- **The slicer seam channel runs down the tactile arrow column the full height and is cut through the raised arrows, and tactile mode recommends 13 cells with a card-fit warning** (2026-09-21, Brennen's decisions D-T1..D-T7 after three printed cards). A printed 14-cell tactile card ran out of paper at the end of every row: the embosser is loaded with the card's leading edge at the alignment arrow, and 14 cells need 92.8 mm of a 90 mm card at any arrow position. The arrow stays at the midpoint of the seam gap, so the space before the first cell equals the space after the last (a fixed lead-in before the first cell was tried and reverted the same day, on Brennen's test of the build). In tactile mode the seam channel now runs down the arrow column itself the full height, and on the embossing plate it is cut a second time through the raised arrows, so the slicer has a corner at every layer (a groove in two stretches that stopped short of the arrows was tried and printed the same day: wherever it stopped, the slicer chose a dot). Each raised arrow keeps its base half as two ridges and loses its point; the counter plate's recesses are unchanged and the notched arrow still nests. The visual-mode groove is unchanged. Every generated tactile cylinder changes; all eight golden pairs were regenerated.

### Added
- **A card-fit warning for tactile rows.** When a layout would run off the card measured from the alignment arrow, the app says so live beside the seam-gap warning and in the generated spec's warnings — a warning, never a refusal (S-T1).
...
### Changed
- **One Generate, one Download: Generate STL now builds both cylinders, and Download STL saves them as one file.** Every embosser needs the pair, so the pair is the default: press Generate STL and Cylinder A then Cylinder B are built from the same settings, then Download STL saves one combined file with both spaced 10 mm apart for a single print plate. To build one cylinder on its own, open Expert Mode and choose it under **Cylinders to Generate**, the new first submenu — the "Select Plate to Generate" box has moved there, with "Both Cylinder A and B" as the default beside the two single choices. The separate Generate Both, Download Combined, Download Cylinder A and Download Cylinder B buttons are gone. Nothing changed in the files themselves or their names (`Cylinder_Pair_…`, `Cylinder_A_…`, `Embossing_Cylinder_…` and the rest are exactly as before), and nothing ever downloads on its own: one press saves one file, and if the combined file cannot be built the Download button saves Cylinder A first and Cylinder B on the next press. Wording signed off by Brennen on 2026-09-21.

### Added
- **Fixed gears for the Version 2 embosser: choose Version 2 and "Simplified: gears fixed to the cylinders" and each cylinder generates as one piece with its Version 2 drive gears attached.** The barrel prints solid with no keyed holes — the gears are already on, so nothing has to be keyed — and the two notches in the top gears' faces are filled with hidden material so no sealed pocket is left inside. The gears are a 1:1 copy of the v8 Version 2 gear models (`static/assets/gears/v2_gears_*`, derived by `scripts/derive_gear_assets_v2.py`, never edited by hand). The cylinder must be the Version 2 preset size, 30.8 × 54 mm; any other size is refused with a sentence naming the limit, and the same sentence appears live under the Gears choice before you generate. Downloads gain both segments: `Embossing_Cylinder_Geared_V2_…`, `Counter_Cylinder_Geared_V2_…`, `Cylinder_Pair_Geared_V2_…`. Version 1 gears, Version 2 without gears and every existing filename are unchanged and byte-identical. A new golden pair (`tests/fixtures/v2_gear_roller{A,B}_golden.*`) pins the one-piece roller; no existing fixture changed. Wording signed off by Brennen on 2026-09-21. Not yet print-tested.

### Changed
- **One "Embosser setup" item at the top of the page now holds the three choices that used to be three toggles in three places, and nothing on the page is labelled BETA or prototype any more.** Embosser version (Version 1 / Version 2), Gears (Standard: print the gears separately / Simplified: gears fixed to the cylinders) and Card sides (Single-sided / Double-sided) are three either/or radio groups with short plain-language notes, and a "Which setup should I choose?" link opens a new Embosser Setup help tab describing the four released embosser projects and the double-sided option. The Double-Sided Card accordion and checkbox and the Integrated Gears checkbox are gone; a saved choice carries over. The Back of Card section is always on the page, greyed out and unavailable until Double-sided is chosen, so it can never be hidden inside a closed menu. Each choice announces itself once to screen readers, the lock note now says "Choose Single-sided to pick visual markers", and choosing Version 2 while Simplified gears are selected puts the gears back to Standard and says so (fixed gears for Version 2 are still to come). The wire, the geometry and the filenames are unchanged. All new wording is draft pending Brennen's sign-off.

### Added
- **Every cylinder now carries a slicer seam channel, so the layer seam never lands on a dot and no seam painting is needed.** A shallow V-groove — 1.0 mm wide, 0.5 mm deep, the full height of the outer surface — sits in the seam gap beside the row markers on both cylinders of a pair. A slicer's default "Aligned" seam mode snaps each layer's seam into that groove instead of into a braille dot: in the 2026-09-20 slicing test it caught 100 % of layers on the visual-marker plates and on the tactile counter plate, and 90.8 % on the tactile embossing plate with the rest on the raised arrow tips and none in a dot, where a plain barrel put 18–24 % of seams into a dot. The groove is on by default; a new **Slicer seam channel** switch under Expert Mode → Surface Dimensions turns it off and is the only thing that changes the request (`seam_channel_enabled: 0`), so an untouched request body is byte-identical to before and switching it off reproduces the previous STL byte for byte. The groove is left out, with a note shown before Generate, when the seam gap has less than 1.5 mm of free surface for it (15 cells in the tactile style) or a cutout would leave under 1.2 mm of wall beneath it. Its size is a print-tuned constant, not a dial. Exports are not rotated: "Back"/"Rear" seam modes cannot be made safe for the embossing plate by any orientation, so the cylinder guide now says to keep the slicer on Aligned. All six golden fixture pairs were regenerated once for the groove (each gained exactly the groove's 8 triangles). Decisions and the groove size (Brennen, 2026-09-20); the on-screen wording is still draft pending his sign-off. The OpenSCAD companions receive the groove in a follow-on plan.
- **The tactile seam arrows now tell the two card thickness presets apart by touch.** In the Tactile seam arrow style, the **0.4 mm** preset keeps one arrow per braille row exactly as before, while the **0.3 mm** preset places **exactly three arrows, evenly spaced** — at the cylinder's mid-height and 15 mm above and below it — whatever the row count. Three separated arrows against a chain of four touching ones can be counted with a fingertip, and a mixed pair will not nest by hand: a 0.4 mm Cylinder A's second and third arrows meet bare surface on a 0.3 mm Cylinder B, and a 0.3 mm Cylinder A's middle arrow stands proud on a 0.4 mm one. The Custom preset follows whichever preset was last chosen, so nudging a dial on a 0.3 mm pair keeps its three arrows, before and after a reload. On the wire this is one new settings field, `tactile_indicator_layout` (`per_row` | `three_spaced`), sent only for the 0.3 preset in tactile mode, so every 0.4 request body is byte-identical to before; a typo, or a `three_spaced` cylinder too short for its outer arrows (under 40.4 mm at the default dials), is refused with a plain message. The double-sided golden pair, which is the 0.3 mm package, was regenerated with the three-arrow layout; the gear and Version 2 pairs were deliberately left byte-identical. Positions, the Custom rule, the rejection and the fixture scope were decided by Brennen on 2026-09-20. The OpenSCAD companions receive the same rule in their own repository.

### Changed
- **The default cylinder returns to 30.8 × 52 mm — the 54 mm card-shelf barrel is now Embosser Version 2 only.** Version 1 is the 52 mm standard: every previously shipped V1 gear model pairs with a 52 mm barrel, and the one-day 54 mm default (its entry below) made the integrated-gears BETA warn and reject on untouched dials. The height default, both card-stock presets, and the absent-field fallback all return to 52, so **the integrated-gears BETA works on untouched dials again**; selecting Version 2 still forces 30.8 × 54 exactly as before, and switching back to Version 1 restores the dials. Version 2 pairs the updated 54 mm model with the re-cut (v7.2) gears, and its OpenSCAD companion (`Braille_Cylinder_STL_Generator_EmbosserV2.scad` in the braille-stl-generator-openscad repo) carries the same 54 mm barrel, the interpoint option, and a four-rows-per-face text input — while the Version 1 `.scad` files keep the 52 mm barrel deliberately, and integrated gears remain the one feature with no Version 2 OpenSCAD counterpart. No golden fixture moved; height still tells Version 2 (54) apart from the gears BETA and the Version 1 default (52).
...
- **Version 2 cylinder preset, as a warning rather than a rule.** Selecting Version 2 sets the cylinder to 30.8 x 52 mm and says so live if you change it, but never refuses the request - the barrel is still under test. Selecting Version 1 again restores the dials you had.
...
### Fixed
- **Third Version 2 print test: the barrel returns to 30.8 mm.** A printed 30.5 x 52 mm double-sided pair felt loose in the machine and left dots that were neither as deep nor as consistent as Version 1's. Those are the same two symptoms the original 30.1 mm pair gave, weaker - two points on one line - so the barrel stopped stepping and went to **30.8 mm**, the size Version 1 has always used. Both symptoms have one cause, too little interference between the pair, and the diameter is the only dial that sets it. **Nothing about the gear fit moves with the barrel**: every keyed hole and every anti-rotation feature sits at a fixed radius from the axis, so the R14 pegs, the nubs and the sockets are untouched. The bigger barrel also relaxes the tightest wall in the design rather than straining it - the anti-rotation socket leaves 1.4025 mm on Cylinder A and 2.1141 mm on Cylinder B, against 1.2525 and 1.9641 before - and widens the seam gap to 5.76 mm. Version 2 and the integrated-gears BETA now ask for the same 30.8 mm cylinder, so the size no longer tells them apart; the two remain mutually exclusive on the version, and two tests that had quietly relied on the sizes differing now set their own. **Version 1 is untouched**, proved byte-identical at the geometry-spec level. The Version 2 golden pair was regenerated; every other fixture is md5-identical.
