...
- **Tactile mode now recommends 13 braille cells per row, not 14.** 14 still fits the seam-gap arithmetic and can be dialled in by hand, but it left the arrow tighter than is comfortable in practice. The visual-marker recommendations are unchanged. This applies to the web app only: the vendored `OpenSCAD/` copy is pinned to upstream v2.4.1 and still says 14 until the wording lands upstream and is re-vendored ([#93](https://github.com/BrennenJohnston/braille-cylinder-stl-generator/issues/93)).
