...
### Changed
- **The slicer seam channel runs down the tactile arrow column, and tactile mode recommends 13 cells with a card-fit warning** (2026-09-21, Brennen's decisions D-T1..D-T6 after a printed card). A printed 14-cell tactile card ran out of paper at the end of every row: the embosser is loaded with the card's leading edge at the alignment arrow, and 14 cells need 92.8 mm of a 90 mm card at any arrow position. The arrow stays at the midpoint of the seam gap, so the space before the first cell equals the space after the last (a fixed lead-in before the first cell was tried and reverted the same day, on Brennen's test of the build). In tactile mode the seam channel now runs down the arrow column itself, in two stretches that stop 0.3 mm short of the arrow chain, so the seam gap is used evenly and the arrows' own corners hold the seam across the chain; the visual-mode groove is unchanged. Every generated tactile cylinder changes; all eight golden pairs were regenerated.
