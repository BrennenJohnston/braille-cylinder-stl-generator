...
- **Key clearance dial (Expert Mode).** 0.075 mm per side by default, adjustable 0-0.5 mm in steps of 0.005. It grows each of the four keyed holes outward; the nub on Cylinder A does not move with it, because the gear that mates with the nub is already cut. The keys stay error-proof across the whole range: every peg enters its own hole and no other, in either rotation, with a smallest wrong-pair margin of 0.925 mm at the default and 0.500 mm at the maximum.
